            ...
            <div class="row no-gutters py-2">
              ...
              <div class="col-md-3 col-sm-6">
                <div class="card-counter bg-success">
                  <i class="fa fa-newspaper-o"></i>
                  <span class="count-numbers">
                    <a class="count_number_a" href="{{route('admin.users.index', ['is_content_creator' => CONTENT_CREATOR])}}">{{$data->total_content_creators ?: tr('n_a')}}
                    </a>
                  </span>
                  <span class="count-name">{{tr('content_creators')}}</span>
                </div>
              </div>
              @endif
              <div class="col-md-3">
    ...
    <div class="card">
      ...
      <div class="card-body graph-scroll-body">
        ...
        <div class="card-header">
          ...
          <h4 class="card-title">{{tr('post_analytics')}}</h4>
        ...
        <div class="media-list media-list-hover media-list-divided inner-user-div" style="height: 345px"><hr>
          ...
          <a href="{{route('admin.users.view',['user_id'=>$user->id])}}">
            ...
            <div class="media-body">
              ...
              <small class="text-fader">{{common_date($user->created_at, Auth::guard('admin')->user()->timezone) ?: tr('n_a')}}</small>
            </div>
        ...
        <a href="{{route('admin.users.view',['user_id'=>$recent_premium_user->id])}}">
          ...
          <div class="media-body">
            <h6> {{ $recent_premium_user->name ?: tr('n_a') }}</h6>
              <small class="text-fader">{{common_date($recent_premium_user->created_at, Auth::guard('admin')->user()->timezone) ?: tr('n_a')}}</small>
          </div>
          ...
          <a  href="{{route('admin.posts.view',['post_id'=>$post->id])}}">
            ...
            <div class="media media-single">
              ...
              <div class="media-body" style="height: 45px; overflow: hidden;">
                ...
                <small class="text-fader">{{common_date($post->created_at ,Auth::guard('admin')->user()->timezone) ?: tr('n_a')}}</small>
              </div>
          ...
          <a href="{{route('admin.post.payments.view',['post_payment_id'=>$post_payment->id])}}">
            ...
            <div class="media media-single">
              ...
              <div class="media-body">
                ...
                <small class="text-fader">{{common_date($post_payment->created_at, Auth::guard('admin')->user()->timezone) ?: tr('n_a')}}</small>
              </div>
...
<script type="text/javascript">
  $(window).on("load", function() {
    ...
    Morris.Area({
        ...
        data: [<?php foreach ($data->posts_data as $key => $value) {
          echo '{'."period:'". $value->month ."',no_of_posts:". $value->no_of_posts.'}' . ',';
        }?>],
        xkey: 'period',
        ykeys: ['no_of_posts'],
        labels: ['No of Posts'],
