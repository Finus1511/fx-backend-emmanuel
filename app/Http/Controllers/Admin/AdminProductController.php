...
class AdminProductController extends Controller
{
    ...
    public function user_products_index(Request $request) {
        ...
        if($request->user_id){
         ...
         if($request->is_outofstock != '') {

            if($request->is_outofstock == OUT_OF_STOCK){

                $base_query = $base_query->whereIn('user_products.is_outofstock',[OUT_OF_STOCK]);

                
            }else{

                $base_query = $base_query->where('user_products.is_outofstock',$request->is_outofstock);
    ...
    public function product_categories_save(Request $request) {
        ...
        try {
            ...
            $rules = [
                'name' => $request->category_id ? 'required|max:191|unique:product_categories,name,'.$request->category_id.',id' : 'required|max:191|unique:product_categories,name,NULL,id',
                'picture' => 'mimes:jpg,png,jpeg',
                'discription' => 'max:199',
            ];
    ...
    public function product_sub_categories_save(Request $request) {
        ...
        try {
            ...
            $rules = [
                'name' => $request->category_id ? 'required|max:191|unique:product_sub_categories,name,'.$request->category_id.',id' : 'required|max:191|unique:product_sub_categories,name,NULL,id',
                'picture' => 'mimes:jpg,png,jpeg',
                'discription' => 'max:199',
