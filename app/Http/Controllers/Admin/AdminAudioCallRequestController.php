...
class AdminAudioCallRequestController extends Controller
{
    ...
    public function audio_call_payment_excel(Request $request) {
     ...
     /**
     * @method audio_call_payments_send_invoice
     *
    ...
    public function audio_call_payments_send_invoice(Request $request) {

        try {

            $audio_call_payment = \App\Models\AudioCallPayment::where('id', $request->audio_call_payment_id)->first();
            ...
            $user = \App\Models\User::find($audio_call_payment->user_id);
            ...
            $email_data['audio_call_payments'] =  $audio_call_payment ?? "";

            $email_data['user'] = $user ?? '';

            $email_data['audio_call'] = $audio_call_payment->Audiocallrequest ?? '';

            $email_data['amount'] = formatted_amount($audio_call_payment->user_amount) ?? 0;

            if(Setting::get('is_only_wallet_payment')) {
                
                $email_data['amount'] = formatted_amount($audio_call_payment->user_token) ?? 0;
            }

            $email_data['subject'] =  tr('audio_call_payment_invoice_message')." ".Setting::get('site_name');
