...
class AdminLookupController extends Controller
{
     ...
     *
     */
    public function inapp_purchases_index(Request $request) {

        $inapp_purchases = \App\Models\InappPurchase::orderBy('created_at', 'desc')->paginate($this->take);

        return view('admin.inapp_purchases.index')
                    ->with('page','inapp_purchases')
                    ->with('sub_page', 'inapp_purchases-view')
                    ->with('inapp_purchases', $inapp_purchases);
    }

    /**
     * @method inapp_purchases_create()
     *
     * @uses To create inapp_purchases details
     *
     * @created  Akshata
     *
     * @updated 
     ...
     *
     */
    public function inapp_purchases_create() {

        $inapp_purchase = new \App\Models\InappPurchase;

        return view('admin.inapp_purchases.create')
                    ->with('page', 'inapp_purchases')
                    ->with('sub_page','inapp_purchases-create')
                    ->with('inapp_purchase', $inapp_purchase);           
    }

    /**
     * @method inapp_purchases_edit()
     *
     * @uses To display and update documents details based on the document id
     *
     * @created Akshata
     ...
     *
     */
    public function inapp_purchases_edit(Request $request) {

        try {

            $inapp_purchase = \App\Models\InappPurchase::find($request->inapp_purchase_id);

            if(!$inapp_purchase) { 

                throw new Exception(tr('inapp_purchase_not_found'), 101);
            }

            return view('admin.inapp_purchases.edit')
                    ->with('page', 'inapp_purchases')
                    ->with('sub_page','inapp_purchases-create')
                    ->with('inapp_purchase' , $inapp_purchase); 
            ...
            return redirect()->route('admin.inapp_purchases.index')->with('flash_error', $e->getMessage());
        }
    
    }

    /**
     * @method inapp_purchases_save()
     *
     * @uses To save the document details of new/existing document object based on details
     *
     * @created Akshata
     ...
     *
     */
    public function inapp_purchases_save(Request $request) {
        ...
        try {
            ...
            $rules = [
                'reference_name' => 'required|max:191',
                'inapp_purchase_id' => 'exists:inapp_purchases,id|nullable',
            ];

            Helper::custom_validator($request->all(),$rules);

            $inapp_purchase = \App\Models\InappPurchase::find($request->inapp_purchase_id) ?? new \App\Models\InappPurchase;

            if($inapp_purchase->id) {

                $message = tr('inapp_purchase_updated_success'); 

            } else {

                $message = tr('inapp_purchase_created_success');

            }

            $inapp_purchase->reference_name = $request->reference_name ?: $inapp_purchase->reference_name;

            $inapp_purchase->amount = $request->amount ?: $inapp_purchase->amount;

            $inapp_purchase->product_id = $request->product_id ?: $inapp_purchase->product_id;

            $inapp_purchase->apple_id = $request->apple_id ?: $inapp_purchase->apple_id;

            if($inapp_purchase->save()) {

                DB::commit(); 

                return redirect(route('admin.inapp_purchases.view', ['inapp_purchase_id' => $inapp_purchase->id]))->with('flash_success', $message);

            } 

            throw new Exception(tr('inapp_purchase_save_failed'));
     ...
     *
     */
    public function inapp_purchases_view(Request $request) {
        ...
        try {
            ...
            return view('admin.inapp_purchases.view')
                        ->with('page', 'inapp_purchases') 
                        ->with('sub_page', 'inapp_purchases-view') 
                        ->with('inapp_purchase', $inapp_purchase);
     ...
     *
     */
    public function inapp_purchases_delete(Request $request) {
        ...
        try {
            ...
            if($inapp_purchase->delete()) {

                DB::commit();

                return redirect()->route('admin.inapp_purchases.index',['page'=>$request->page])->with('flash_success',tr('inapp_purchase_deleted_success'));   

            } 
            
            throw new Exception(tr('inapp_purchase_delete_failed'));
     ...
     *
     **/
    public function inapp_purchases_status(Request $request) {
        ...
        try {
            ...
            $inapp_purchase->status = $inapp_purchase->status ? DECLINED : APPROVED ;

            if($inapp_purchase->save()) {

                DB::commit();

                $message = $inapp_purchase->status ? tr('inapp_purchase_approve_success') : tr('inapp_purchase_decline_success');

                return redirect()->back()->with('flash_success', $message);
            }
            
            throw new Exception(tr('inapp_purchase_status_change_failed'));
