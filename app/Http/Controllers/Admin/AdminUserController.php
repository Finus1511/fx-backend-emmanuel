...
class AdminUserController extends Controller
{
    ...
    public function referrals_view(Request $request) {
        ...
        try {
            ...
            throw_if(!$referrals_code_details, new Exception(tr('referral_details_not_found'), 101));

            $referrals_user_details = \App\Models\UserReferral::where("referral_code_id", "=", $request->referral_id)->paginate($this->take);
     ...
     **/
     ...
     /**
     * @method featured_users_index()
     *
     * @uses to list the featured users details
     *
     * @created Shakthi
     *
     * @updated  
     ...
     * @param
     *
     * @return to featured users index page
     */
    public function featured_users_index(Request $request)
    {
        try {
            ...
            $featured_users = $base_query->latest()->paginate($this->take);

            return view('admin.users.featured_users.index')
                ->with('page', 'featured_users')
                ->with('featured_users', $featured_users);
   ...
   /**
     * Update user to Featured User based on user's ID
     *
     * @param \Illuminate\Http\Request $request
     * @return \Illuminate\Http\Response
     */
    public function marked_as_featured_user(Request $request)
    {
        try {
            ...
            throw_if(!$user, new Exception(tr('user_not_found'), 101));

            throw_if($user->is_content_creator != CONTENT_CREATOR, new Exception(tr('user_not_content_creator'), 102));

            throw_if($user->is_featured_user, new Exception(tr('already_featured_user'), 102));

            $user->is_featured_user = YES;

            if($user->save()) {
 
                DB::commit();

                return redirect()->back()->with('flash_success', tr('marked_as_featured_user_success'));
            }
            
            throw new Exception(tr('marked_as_featured_user_failed'));
     ...
     * @return \Illuminate\Http\Response
     */
    public function remove_featured_user(Request $request)
    {
        try {
            ...
            $user->is_featured_user = NO;

            if($user->save()) {
 
                DB::commit();

                return redirect()->back()->with('flash_success', tr('remove_featured_user_success'));
            }
            
            throw new Exception(tr('remove_featured_user_failed'));
