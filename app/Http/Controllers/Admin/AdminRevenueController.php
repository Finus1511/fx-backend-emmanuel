...
use App\Exports\{TipPaymentExport, PostPaymentExport, UserWalletPaymentExport, OrderPaymentExport};
...
class AdminRevenueController extends Controller
{
     ...
     **/
     ...
     public function order_payments_send_invoice(Request $request) {
        ...
        /**
     * @method wallet_payments_index()
     *
     * @uses to list the Wallet payments
     *
     * @created Shakthi
     *
     * @updated
     ...
     * @param
     *
     * @return to wallet payments index page
     */

    public function wallet_payments_index(Request $request) {

        try {

            $wallet_payments = $user = '';

            $base_query = UserWalletPayment::orderBy('created_at','DESC');

            if($request->filled('search_key')) {

                $base_query = $base_query->when($request->filled('user_id'), function ($query) use ($request) {
                                        $query->where('user_id', $request->user_id);
                                    })->where(function($query) use ($request) {
                                        $query->where('payment_id', "LIKE", "%" . $request->search_key . "%");
                                    })->orWhereHas('ReceivedFromUser', function ($query) use ($request) {
            ...
            if($request->filled('amount_type')) {
                ...
                $base_query = $base_query->where('amount_type', $request->amount_type);
            ...
            if($request->filled('payment_type')) {
                ...
                $base_query = $base_query->where('payment_type', $request->payment_type);
                
            } 

            $wallet_payments = $base_query->orderBy('id', 'DESC')->paginate($this->take);

            return view('admin.wallet_payments.index')
                        ->with('page', 'wallet_payments')
                        ->with(compact(['wallet_payments', 'user']));
    ...
    /**
     * @method wallet_payment_view()
     *
     * @uses To list out users wallet payment details 
     *
     * @created Shakthi
     *
     * @updated 
     ...
     *
     */
    public function wallet_payment_view(Request $request) {

        try {
       
            $wallet_payment = UserWalletPayment::find($request->user_wallet_payment_id);

            throw_if(!$wallet_payment, new Exception(tr('user_not_found'), 101));

            return view('admin.wallet_payments.view')
                        ->with('page', 'wallet_payments')
                        ->with('sub_page', 'wallet-payments')
                        ->with('wallet_payment', $wallet_payment);
    ...
    /**
     * @method wallet_payments_excel()
     *
     * @uses To Export Wallet Payment details 
     *
     * @created Shakthi
     *
     * @updated 
     ...
     *
     */
    public function wallet_payments_excel(Request $request) {
        ...
        try{
            ...
            return Excel::download(new UserWalletPaymentExport($request), $filename);
