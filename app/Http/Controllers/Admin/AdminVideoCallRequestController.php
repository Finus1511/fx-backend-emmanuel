...
class AdminVideoCallRequestController extends Controller
{
    ...
    public function video_call_payment_excel(Request $request) {
     ...
     /**
     * @method video_call_payments_send_invoice
     *
    ...
    public function video_call_payments_send_invoice(Request $request) {
        ...
        try {
            ...
            $user = \App\Models\User::find($video_call_payment->user_id);
            ...
            $email_data['video_call_payments'] =  $video_call_payment ?? "";

            $email_data['user'] = $user ?? '';

            $email_data['video_call'] = $video_call_payment->videocallDetails ?? '';

            $email_data['amount'] = formatted_amount($video_call_payment->user_amount) ?? 0;

            if(Setting::get('is_only_wallet_payment')) {
                
                $email_data['amount'] = formatted_amount($video_call_payment->user_token) ?? 0;
            }

            $email_data['subject'] =  tr('video_call_payment_invoice_message')." ".Setting::get('site_name');
