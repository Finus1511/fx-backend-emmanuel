...
class AdminPromoCodeController extends Controller
{
    ...
    public function promo_codes_save(Request $request) {
        ...
        try {
            ...
            DB::beginTransaction();
            $validator = [
                'promo_code' => $request->promo_code_id ? 'required|max:10|min:4|unique:promo_codes,promo_code,'.$request->promo_code_id : 'required|unique:promo_codes,promo_code|min:4|max:10',
            ...
            $promo_code->promo_code = strtoupper($request->promo_code);

            $promo_code->amount_type = $request->amount_type;

            $promo_code->amount = $request->amount;

            $promo_code->start_date = common_server_date($request->start_date, $request->timezone?: DEFAULT_TIMEZONE , 'Y-m-d H:i:s');

            $promo_code->expiry_date = $request->expiry_date ? common_server_date($request->expiry_date, $request->timezone?: DEFAULT_TIMEZONE, 'Y-m-d H:i:s') : NULL;
            ...
            $promo_code->platform = $request->platform;
