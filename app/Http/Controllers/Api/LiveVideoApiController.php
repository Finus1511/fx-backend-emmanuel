...
class LiveVideoApiController extends Controller
{
    ...
    public function live_videos_payment_by_card(Request $request) {
        ...
        try {
            ...
            $total = $live_video_details->amount ?? 0.00;

            $user_pay_amount = Helper::apply_promo_code($request, $total, LIVE_VIDEO_PAYMENTS, $live_video_details->user_id);
    ...
    public function live_videos_payment_by_wallet(Request $request) {
        ...
        try {
            ...
            $user_pay_amount = Helper::apply_promo_code($request, $total, LIVE_VIDEO_PAYMENTS, $live_video->user_id);
...
$live_video->agora_token = $agora_configured ? ($token ?? '') : '';
            ...
            if($request->hasFile('snapshot')) {
                ...
                $archive_request = new Request(['user_id' => $request->id,'file' => $live_video->snapshot, 'origin' => FILE_ORIGIN_LIVE_VIDEO, 'file_type' => FILE_TYPE_IMAGE, 'live_video_id' => $live_video->id]);
    ...
    /** 
     * @method live_videos_host_update()
     *
     ...
     *
     */
    public function live_videos_host_update(Request $request) {

        try {
            DB::beginTransaction();

            $rules = [
                'live_video_unique_id' => 'required|exists:live_videos,unique_id',
                'host_id' => 'required'
            ];
            ...
            throw_if(!$live_video_details, new Exception(api_error(135), 135));

            DB::commit(); 

            $live_video_details->update(['host_id' => $request->host_id]);
