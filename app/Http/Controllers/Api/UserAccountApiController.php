...
class UserAccountApiController extends Controller
{
        ...
        try {
            ...
            if(in_array($request->login_by, $allowed_social_logins)) {
                ...
                $rules = [
                    ...
                    'picture' => '',
                    'gender' => 'nullable|in:male,female,others,rather-not-select,couple',
                ];
    ...
    public function feature_story_save(Request $request) {
        ...
        try {
            ...
            if($request->hasFile('file') != "") {
                ...
                $archive_request = new Request(['user_id' => $request->id, 'file' => $user->featured_story, 'origin' => FILE_ORIGIN_STORY, 'file_type' => $request->file_type ?? FILE_TYPE_IMAGE]);
    ...
    public function user_billing_accounts_delete(Request $request) {
        ...
        try {
            ...
            $user_billing_account = \App\Models\UserBillingAccount::find($request->user_billing_account_id);

            if($user_billing_account->is_default == YES) {

                $new_default_billing_account = \App\Models\UserBillingAccount::firstWhere(['user_id' => $request->id, 'is_default' => NO]);
            ...
            return $this->sendResponse(api_success(113), 113, '');
    ...
    public function user_subscriptions_payment_by_stripe(Request $request) {
        ...
        try {
            ...
            $user_pay_amount = Helper::apply_promo_code($request, $total, SUBSCRIPTION_PAYMENTS, $user_subscription->user_id);
    ...
    public function content_creators_dashboard(Request $request) {
        ...
        try {
            ...
            $live_stream_shopping = LiveStreamShopping::firstWhere(['user_id' => $request->id]);

            if($live_stream_shopping){

                $user_product_ids = LssProduct::where('live_stream_shopping_id', $live_stream_shopping->id)->pluck('user_product_id')->toArray();

                $live_stream_shopping_payments = LssPayment::where(['live_stream_shopping_id' => $live_stream_shopping->id, 'status' => PAID])->sum('amount');

                $lss_product_payments = LssProductPayment::whereIn('user_product_id', $user_product_ids)->where('status', PAID)->sum('amount');

            }

            $live_video_payments = LiveVideoPayment::where(['user_id' => $request->id, 'status' => PAID])->sum($sum_value);

            $data->live_stream_shopping_payments = formatted_amount($live_stream_shopping_payments ?? 0.00);

            $data->lss_product_payments = formatted_amount($lss_product_payments ?? 0.00);
    ...
    public function user_category_listing(Request $request) {
     ...
     /** 
     * @method featured_users_list()
     *
     * @uses List of featured users
     *
     * @created RA Shakthi
     *
     * @updated 
     ...
     *
     */
    public function featured_users_list(Request $request)
    {
        try {

            $base_query = User::where('is_featured_user', YES)->when($request->filled('search_key'), function ($query) use ($request) {
            ...
            $data['featured_users'] = UserPreviewResource::collection($featured_users);
   ...
   /**
     * Toggle visibility of user's followers alternatively.
     *
     * Created by RA Shakthi
     * 
