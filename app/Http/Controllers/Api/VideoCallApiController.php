...
class VideoCallApiController extends Controller
{
    ...
    /** 
     * @method video_call_requests_start()
     *
    ...
    public function video_call_requests_start(Request $request) {

        try {

            $rules = [
                'video_call_request_unique_id' => 'required|exists:video_call_requests,unique_id,model_id,'.$request->id,
            ];
           
            Helper::custom_validator($request->all(),$rules);

            $video_call_request = VideoCallRequest::where('unique_id',$request->video_call_request_unique_id)->first();
            ...
            $job_data['status'] = VIDEO_CALL_REQUEST_STARTED;

            $job_data['timezone'] = $this->timezone;

            $this->dispatch(new \App\Jobs\VideoCallRequestStartJob($job_data));
    ...
    public function video_call_payment_by_stripe(Request $request) {
        ...
        try {
            ...
            $total = $model->video_call_amount ?: 1;

            $user_pay_amount = Helper::apply_promo_code($request, $total, VIDEO_CALL_PAYMENTS, $model->id);
        ...
        try {
            ...
            if($user_pay_amount > 0) {
                ...
                if($wallet_payment_response->success) {
                    ...
                    $this->dispatch(new \App\Jobs\VideoCallPaymentJob($job_data));
