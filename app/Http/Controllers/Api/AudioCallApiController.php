...
use App\Models\{AudioCallRequest, AudioCallPayment};
...
class AudioCallApiController extends Controller
{
    ...
    /** 
     * @method audio_call_requests_start()
     *
     * @uses to accept the  audio call requests
     *
     * @created Arun
    ...
    public function audio_call_requests_start(Request $request) {

        try {

            $rules = [
                'audio_call_request_unique_id' => 'required|exists:audio_call_requests,unique_id,model_id,'.$request->id,
            ];
           
            Helper::custom_validator($request->all(),$rules);

            $audio_call_request = AudioCallRequest::where('unique_id',$request->audio_call_request_unique_id)->first();
            ...
            $job_data['status'] = AUDIO_CALL_REQUEST_STARTED;

            $job_data['timezone'] = $this->timezone;

            $this->dispatch(new \App\Jobs\AudioCallRequestStartJob($job_data));
    ...
    public function audio_call_payment_by_stripe(Request $request) {
        ...
        try {
            ...
            $total = $model->audio_call_amount ?: 1;

            $user_pay_amount = Helper::apply_promo_code($request, $total, AUDIO_CALL_PAYMENTS, $model->id);
    ...
    public function audio_call_payment_by_wallet(Request $request) {
        ...
        try {
            ...
            if(!$user_wallet) {
                ...
                throw new Exception(api_error(282), 282);
            }
            ...
            if($user_pay_amount > 0) {
                ...
                if($wallet_payment_response->success) {
                    ...
                    $this->dispatch(new \App\Jobs\AudioCallPaymentJob($job_data));
