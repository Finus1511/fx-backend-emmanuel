            ...
            if($post->save()) {
                ...
                if($request->post_file_id) {
                    ...
                    foreach ($files as $key => $post_file_id) {
                        ...
                        $archive_request = new Request(['user_id' => $request->id,'file' => $post_file->file, 'origin' => FILE_ORIGIN_POST, 'file_type' => $post_file->file_type, 'post_id' => $post->id]);
                ...
                $this->dispatch(new \App\Jobs\PostNotificationToFollower($request->id,$post));
    ...
    public function posts_payment_by_stripe(Request $request) {
        ...
        try {
           ...
           // Check the subscription is available
            ...
            $user_pay_amount = Helper::apply_promo_code($request, $total, POST_PAYMENTS, $post->user_id);
