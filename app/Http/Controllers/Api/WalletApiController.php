...
class WalletApiController extends Controller
{
    ...
    public function user_wallets_add_money_by_stripe(Request $request) {
        ...
        try {
            ...
            $is_paid = Helper::check_stripe_payment($request->payment_id);
    ...
    /**
     * @method user_wallets_add_money_by_stripe_ios()
     *
     * @uses Add money by stripe IOS
     *
     * @created Bhawya N
     *
     * @updated Bhawya N
    ...
    public function user_wallets_add_money_by_stripe_ios(Request $request) {
        ...
        try {
            ...
            if($amount > 0) {
                ...
                $data['ephemeralKey']= $ephemeralKey->secret;
                $data['customer'] = $customer->id;
                $data['publishableKey']= Setting::get('stripe_publishable_key');

                $request->request->add([
                    'paid_amount' => $amount, 
                    'paid_status' => USER_WALLET_PAYMENT_UNPAID,
                    'usage_type' => USAGE_TYPE_RECHARGE
                ]);
            ...
            if($payment_response->success) {
                ...
                $data['user_wallet_payment_id'] = $payment_response->data->user_wallet_payment_id;
    ...
    /**
     * @method user_wallets_add_money_by_stripe_ios_status()
     *
     * @uses
     *
     * @created 
    ...
    public function user_wallets_add_money_by_stripe_ios_status(Request $request) {
        ...
        try {
            ...
            $rules = [
                'user_wallet_payment_id' => 'required|integer|exists:user_wallet_payments,id',
            ];

            Helper::custom_validator($request->all(), $rules, $custom_errors = []);

            $user_wallet_payment = \App\Models\UserWalletPayment::where('user_id',$request->id)->where('id', $request->user_wallet_payment_id)->first();

            $user_wallet_payment->status = $request->status;
            ...
            $message = $request->status ? api_success(118) : api_error(266);
    ...
    /**
     * @method user_wallets_add_money_by_inapp()
     *
     * @uses Inapp purchase update
     *
     * @created Vithya R
     *
     * @updated Vithya R
    ...
    public function user_wallets_add_money_by_inapp(Request $request) {
        ...
        try {
            ...
            $request->request->add(['tokens' => $request->tokens ?: $request->amount]);
