...
class UserProductApiController extends Controller
{
    ...
    public function ecommerce_home(Request $request) {
        ...
        try {
            ...
            $base_query = $total_query = UserProduct::Approved()->where([['user_id', '!=', $request->id],'is_live_stream_product' => NO]);
    ...
    public function user_products_view_for_others(Request $request) {
        ...
        try {
            ...
            $user_product = UserProduct::with('userProductPictures', 'productCategory',
            'productSubCategory', 'user')->whereHas('user')->Approved()->where('user_products.unique_id', $request->user_products_unique_id)->first();
            ...
            $data['user_product'] = new UserProductResource($user_product);
    ...
    public function user_products_save(Request $request) {
        ...
        try {
            ...
            $rules = [
                ...
                'is_digital_product' => ['nullable', Rule::in([YES, NO])],
                'product_file' => $request->is_digital_product == 'YES' ? 'required|mimes:doc,docx,pdf,word,xls,xlsx' : 'nullable|mimes:doc,docx,pdf,word,xls,xlsx',
            ];
            ...
            $user_product->is_digital_product = intval($request->is_digital_product);
            ...
            if($request->hasFile('picture')) {
             ...
             if($request->hasFile('product_file')) {
                ...
                if($request->user_product_id) {
                    ...
                    Helper::storage_delete_file($user_product->product_file, USER_PRODUCT_FILE_PATH); 
                }

                $user_product->product_file = Helper::storage_upload_file($request->file('product_file'), USER_PRODUCT_FILE_PATH);
            }
            ...
            if($user_product->save()) {
                ...
                $archive_request = new Request(['user_id' => $request->id,'file' => $user_product->picture, 'origin' => FILE_ORIGIN_ECOM, 'file_type' => FILE_TYPE_IMAGE, 'user_product_id' => $user_product->id]);
    ...
    /**
     * @method view_product_file()
     *
     * @uses To display all the product file url
     *
     * @created RA Shakthi
     *
     * @updated 
     ...
     * @return JSON Response
     */
    public function view_product_file(Request $request) {
        ...
        try {
            ...
            throw_if(!$user_product->is_digital_product, new Exception(api_error(283), 283)); 

            $user_needs_to_pay = user_need_to_pay($request->id, $user_product->id);
             
            throw_if($user_needs_to_pay, new Exception(api_error(284), 284));

            throw_if(!Hash::check($request->password, $user->password), new Exception(api_error(108), 108));
            
            $data['product_file'] = $user_product->product_file ?? [];
    ...
    /**
     * @method digital_product_payment_by_wallet()
     *
     * @uses To Pay the digital product 
     *
     * @created RA Shakthi
     *
     * @updated 
     ...
     * @return JSON Response
     */
    public function digital_product_payment_by_wallet(Request $request) {
        ...
        try {
            ...
            $user_product_price = Helper::apply_promo_code($request, $user_product->price, ORDER_PAYMENTS, $user_product->user_id);

            $digital_product_order = PaymentRepo::buy_digital_product($user_product, $this->loginUser->id, $user_product_price);

            throw_if(!$digital_product_order['success'], new Exception($digital_product_order['message']), $digital_product_order['code']);
            
            $request->request->add([
                'to_user_id' => $user_product->user_id,
                'total' => $user_product->price * Setting::get('token_amount'),
                'user_pay_amount' => $user_product->price,
                'paid_amount' => $user_product->price * Setting::get('token_amount'), 
                'payment_id' => 'AC-'.rand(),
                'payment_mode' => PAYMENT_MODE_WALLET,
                'payment_type' => WALLET_PAYMENT_TYPE_PAID,
                'amount_type' => WALLET_AMOUNT_TYPE_MINUS,
                'usage_type' => USAGE_TYPE_ORDER,
                'tokens' => $user_product->price,
            ]);
            ...
            if($wallet_payment_response->success) {
                    ...
                    $data['user_payment_details'] = $wallet_payment_response ?? [];

                    return $this->sendResponse(api_success(813), '', $data);
