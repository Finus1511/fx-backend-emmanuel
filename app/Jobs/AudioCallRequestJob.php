...
class AudioCallRequestJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $content = push_messages(614);

            if($this->data['status'] == AUDIO_CALL_REQUEST_ACCEPTED){

                $message = tr('audio_call_request_approve_message'); 

            } 
            else{
                $message = tr('audio_call_request_reject_message'); 

            }


            $message = str_replace("<%user%>",$audio_call_request->model->name ?? '' ,$message);


            $data['from_user_id'] = $audio_call_request->model_id;

            $data['to_user_id'] = $audio_call_request->user_id;
            ...
            $data['image'] = $audio_call_request->model->picture ?? asset('placeholder.jpeg');

            $data['subject'] = $content;

            dispatch(new BellNotificationJob($data));

            $user = User::where('id', $audio_call_request->user_id)->first();
