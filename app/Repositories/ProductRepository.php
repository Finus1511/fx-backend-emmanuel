...
class ProductRepository {
    ...
    public static function user_product_pictures_save($files, $user_product_id) {
        ...
        if(!is_array($files)) {
            ...
            $user_product_picture = new \App\Models\UserProductPicture;

            $user_product_picture->user_product_id = $user_product_id;

            $user_product_picture->picture = Helper::storage_upload_file($file, COMMON_FILE_PATH);

            $user_product_picture->save();
        ...
        foreach($files as $file) {
            ...
            if($check_picture) {
                ...
                $archive_request = new Request(['user_id' => $request->id,'file' => $user_product_picture->picture, 'origin' => FILE_ORIGIN_ECOM, 'file_type' => FILE_TYPE_IMAGE, 'user_product_id' => $user_product_id]);
