...
use App\Models\{LiveVideoPayment, OrderPayment, UserProduct, Cart, OrderProduct, LssProductPayment, User, SubscriptionPayment, VodPayment, PromoCode, Order, UserWallet};
...
class PaymentRepository {
    ...
    public static function subscriptions_payment_save($request, $subscription) {
        ...
        try {
            ...
            $user_subscription->promo_code = $request->promo_code ?? NULL;

            $user_subscription->promo_discount = $request->promo_discount ?? 0.00;
    ...
    public static function post_payments_save($request, $post, $promo_amount = 0) {
        ...
        try {
            ...
            if ($request->promo_code) {
                ...
                $post_payment->promo_code = $request->promo_code ?: NULL;

                $post_payment->promo_code_amount = $request->promo_discount ?: 0.00;
    ...
    public static function user_subscription_payments_save($request, $user_subscription) {
        ...
        try {
            ...
            if ($request->promo_code) {
                ...
                $user_subscription_payment->promo_code = $request->promo_code;

                $user_subscription_payment->promo_code_amount = $request->promo_discount;
    ...
    public static function chat_assets_payment_save($request, $chat_message) {
        ...
        try {
            ...
            $chat_asset_payment->promo_code = $request->promo_code ?: NULL;

            $chat_asset_payment->promo_discount = $request->promo_discount ?? 0;
            
            $chat_asset_payment->save();

            $chat_message->promo_code = $request->promo_code ?: NULL;

            $chat_message->promo_discount = $request->promo_discount ?? 0;

            $chat_message->user_paid_amount = $request->paid_amount ?? 0.00;
    ...
    public static function video_call_payments_save($request) {
        ...
        try {
            ...
            $video_call_payment->promo_code = $request->promo_code ?: NULL;

            $video_call_payment->promo_discount = $request->promo_discount ?? 0;
    ...
    public static function live_videos_payment_save($request, $live_video_details) {
        ...
        try {
            ...
            $live_video_payment->promo_code = $request->promo_code ?? NULL;

            $live_video_payment->promo_discount = $request->promo_discount ?? 0.00;
    ...
    public static function audio_call_payments_save($request) {
        ...
        try {
            ...
            $audio_call_payment->promo_code = $request->promo_code ?: NULL;

            $audio_call_payment->promo_discount = $request->promo_discount ?? 0;
    ...
    public static function order_payments_save($request, $order) {
        ...
        try {
            ...
            // $order_payment->promo_code = $request->promo_code ?: NULL;

            // $order_payment->promo_discount = $request->promo_discount ?? 0;
    ...
    /**
     * @method buy_digital_product
     *
     * @uses Buy digital product
     *
     * @created RA Shakthi
     *
     * @updated 
    ...
    public static function buy_digital_product($user_product, $user_id, $user_product_price) {
        ...
        try {   
            ...
            throw_if($user_wallet->remaining < $user_product_price, new Exception(api_error(142), 142));

            $order_product_exists = OrderProduct::where(['user_product_id' => $user_product->id, 'user_id' => $user_id])->exists();

            throw_if($order_product_exists, new Exception(api_error(285, $user_product->name), 285));

            $order = Order::create([
                'user_id' => $user_id,
                'sub_total' => $user_product_price,
                'total' => $user_product_price,
            ]);

            $token = $user_product_price * Setting::get('token_amount');
            ...
            $order_product = OrderProduct::create([
                ...
                'quantity' => 1,
                'per_quantity_price' => $user_product_price,
                'sub_total' => $user_product_price,
                'delivery_price' => $user_product->delivery_price,
                'total' => $user_product_price,
                'token' => $token,
                'status' => PAID
            ]);

            $order_product = OrderPayment::create([
                'user_id' => $user_id,
                'order_id' => $order->id,
            ...
            $response = ['success' => true, 'message' => '', 'code' => '', 'data' => []];
     ...
     /**
     * @method lss_product_payment_wallet_update
     *
     * @uses post payment amount will update to the live stream product owner wallet
     *
     * @created RA Shakthi
     *
     * @updated 
    ...
    public static function lss_product_payment_wallet_update($request, $user_product, $lss_product_payment) {

        try {

            $to_user_inputs = [
                'id' => $user_product->user_id,
                'received_from_user_id' => $lss_product_payment->user_id,
                'total' => $lss_product_payment->paid_amount ?? 0.00, 
                'user_pay_amount' => $lss_product_payment->user_amount ?? 0.00,
                'paid_amount' => $lss_product_payment->user_amount ?? 0.00,
                'payment_type' => WALLET_PAYMENT_TYPE_CREDIT,
                'amount_type' => WALLET_AMOUNT_TYPE_ADD,
                'payment_id' => $lss_product_payment->payment_id,
                'admin_amount' => $lss_product_payment->admin_amount ?? 0.00,
                'user_amount' => $lss_product_payment->user_amount ?? 0.00,
                'usage_type' => USAGE_TYPE_PPV,
                'user_token' => $lss_product_payment->amount ?? 0.00,
                'admin_token' => $lss_product_payment->admin_amount ?? 0.00,
                'tokens' => $lss_product_payment->user_amount ?? 0.00, 
            ];

            $to_user_request = new Request($to_user_inputs);

            $to_user_payment_response = self::user_wallets_payment_save($to_user_request)->getData(); // need to review //this is called before
    ...
    /**
     * @method lss_payment_wallet_update
     *
     * @uses post payment amount will update to the live stream owner wallet
     *
     * @created RA Shakthi
     *
     * @updated 
    ...
    public static function lss_payment_wallet_update($request, $live_stream_shopping, $lss_payment) {

        try {

            $to_user_inputs = [
                'id' => $live_stream_shopping->user_id,
                'received_from_user_id' => $lss_payment->user_id,
                'total' => $lss_payment->paid_amount ?? 0.00, 
                'user_pay_amount' => $lss_payment->user_amount ?? 0.00,
                'paid_amount' => $lss_payment->user_amount ?? 0.00,
                'payment_type' => WALLET_PAYMENT_TYPE_CREDIT,
                'amount_type' => WALLET_AMOUNT_TYPE_ADD,
                'payment_id' => $lss_payment->payment_id,
                'admin_amount' => $lss_payment->admin_amount ?? 0.00,
                'user_amount' => $lss_payment->user_amount ?? 0.00,
                'usage_type' => USAGE_TYPE_PPV,
                'user_token' => $lss_payment->amount ?? 0.00,
                'admin_token' => $lss_payment->admin_amount ?? 0.00,
                'tokens' => $lss_payment->user_amount ?? 0.00, 
            ];
