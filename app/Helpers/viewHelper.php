 ...
 */
function promo_calculation($amount,$request,$promo_code = [], $user_details = []) {
...
function following_users($user_id){
 ...
 function placeholder_path_formate($key) {
    ...
    $data['file_name'] = isset($status_list[$key]) ? $status_list[$key] : $key;
...
function virtual_experience_status_text_formatted($status) {

    $status_list = [
        VIRTUAL_EXPERIENCE_CREATED => tr('VIRTUAL_EXPERIENCE_CREATED'), 
        VIRTUAL_EXPERIENCE_STARTED => tr('VIRTUAL_EXPERIENCE_STARTED'),
        VIRTUAL_EXPERIENCE_COMPLETED => tr('VIRTUAL_EXPERIENCE_COMPLETED'), 
        VIRTUAL_EXPERIENCE_CANCELLED => tr('VIRTUAL_EXPERIENCE_CANCELLED'),
        VIRTUAL_EXPERIENCE_FULL => tr('VIRTUAL_EXPERIENCE_FULL')
    ];

    return isset($status_list[$status]) ? $status_list[$status] : tr('VIRTUAL_EXPERIENCE_STARTED');
}

function start_call_formatted($startTime, $endTime, $timezone) {

    $start = date('Y-m-d H:i:s',strtotime('-'.(5).' mins',strtotime($startTime)));

    $end = date('Y-m-d H:i:s',strtotime('-'.(5).' mins',strtotime($endTime)));

    $current_date = strtotime(common_server_date(date("Y-m-d H:i:s"),"", "Y-m-d H:i:s"));

    return $current_date >= strtotime($start) && $current_date <= strtotime($end) ? YES : NO;
}


function creator_check_formatted($user_id) {

    return User::where(['id' => $user_id, 'is_content_creator' => CONTENT_CREATOR])->exists() ? YES : NO;
}

function virtual_experience_creator_check($virtual_experience_id, $user_id) {

    $virtual_experience = VirtualExperience::where(['id' => $virtual_experience_id, 'user_id' => $user_id])->first();

    return $virtual_experience ? YES : NO;
}

function is_creator_needs_to_pay($virtual_experience_id, $user_id) {

     $virtual_experience_creator = virtual_experience_creator_check($virtual_experience_id, $user_id);

    if(!$virtual_experience_creator){

      $virtual_experience_payment = VirtualExperienceBooking::where(['virtual_experience_id' => $virtual_experience_id, 'user_id' => $user_id,  'status' => VIRTUAL_EXPERIENCE_PAID])->first();
    }

    return $virtual_experience_creator == YES ? NO : ($virtual_experience_payment ? NO : YES);
}

function user_need_to_pay($user_id, $user_product_id) {

    $order_product = OrderProduct::where(['user_id' => $user_id, 'user_product_id' => $user_product_id])->first();
    
    if ($order_product) {

        $order_payment = OrderPayment::where(['user_id' => $user_id, 'order_id' => $order_product->order_id])->first();
        
        return $order_payment ? NO : YES;
    }
...
function personalize_status_formatted($status) {

    $status_list = [
        PERSONALIZE_USER_REQUESTED => tr('user_requested'), 
        PERSONALIZE_CREATOR_ACCEPTED => tr('creator_accepted'),
        PERSONALIZE_CREATOR_REJECTED => tr('creator_rejected'), 
        PERSONALIZE_USER_REJECTED => tr('user_rejected'),
        PERSONALIZE_USER_PAID => tr('user_paid'),
        PERSONALIZE_CREATOR_UPLOADED => tr('creator_uploaded'),
        PERSONALIZE_USER_CANCELLED => tr('user_cancelled'),
    ];

    return isset($status_list[$status]) ? $status_list[$status] : tr('user_requested');
}
 ...
 * @return integer
*/
function product_type_formatted($type) {

    $type_list = [
        PRODUCT_TYPE_NONE => tr('none'), 
        PRODUCT_TYPE_DIGITAL => tr('digital'),
        PRODUCT_TYPE_PHYSICAL => tr('physical'),
    ];

    return isset($type_list[$type]) ? $type_list[$type] : tr('none');
}

function personalized_status_badge_formatted($status) {

    $status_list = [

        PERSONALIZE_USER_REQUESTED => "badge badge-primary",
        PERSONALIZE_CREATOR_ACCEPTED=> "badge badge-success",
        PERSONALIZE_CREATOR_REJECTED => "badge badge-danger",
        PERSONALIZE_USER_REJECTED => "badge badge-danger",
        PERSONALIZE_USER_REJECTED => "badge badge-secondary",
        PERSONALIZE_USER_PAID => "badge badge-success",
        PERSONALIZE_USER_CANCELLED => "badge badge-warning",
...
function check_user_needs_to_pay($live_stream_shopping_id, $user_id) {

    $is_user_needs_to_pay = NO;

     $is_user_needs_to_pay = LssPayment::where(['live_stream_shopping_id' => $live_stream_shopping_id,'user_id' => $user_id, 'status' => PAID])->count() ? NO : YES;
 ...
 * @return integer
*/
function live_stream_status_formatted($status) {

    $status_list = [
        LIVE_STREAM_SHOPPING_CREATED => tr('live_stream_created'), 
        LIVE_STREAM_SHOPPING_ONGOING => tr('live_stream_ongoing'),
        LIVE_STREAM_SHOPPING_COMPLETED => tr('live_stream_completed'),
    ];

    return isset($status_list[$status]) ? $status_list[$status] : tr('live_stream_created');
}


function live_stream_status_badge_formatted($status) {

    $status_list = [
        LIVE_STREAM_SHOPPING_CREATED => "badge badge-primary",
        LIVE_STREAM_SHOPPING_ONGOING=> "badge badge-success",
        LIVE_STREAM_SHOPPING_COMPLETED => "badge badge-danger",
    ];

    return isset($status_list[$status]) ? $status_list[$status] : 'badge badge-primary';
}

function get_overall_revenue($live_stream_shopping_id, $user_id) {

     $total_revenue = LssProductPayment::where('user_id', '!=', $user_id)->where(['live_stream_shopping_id' => $live_stream_shopping_id, 'status' => PAID])->sum('amount');

    return isset($total_revenue) ? formatted_amount($total_revenue) : 0.00;
}

function check_call_status($live_stream_shopping_id, $user_id, $startTime, $timezone) {

    $start = date('Y-m-d H:i:s', strtotime('-5 minutes', strtotime($startTime)));

    $current_date = strtotime(now('UTC'));

    $live_stream = LiveStreamShopping::firstWhere(['id' => $live_stream_shopping_id, 'user_id' => $user_id]);

    $is_call_start =  $live_stream->status != LIVE_STREAM_SHOPPING_COMPLETED && $current_date >= strtotime($start) ? YES : NO;

    return $is_call_start;
}

function platform_formatted($platform) {

    $platform_list = [
        SUBSCRIPTION_PAYMENTS => tr('subscription_payments'),
        POST_PAYMENTS => tr('post_payments'),
        VIDEO_CALL_PAYMENTS => tr('video_call_payments'),
        AUDIO_CALL_PAYMENTS => tr('audio_call_payments'),
        CHAT_ASSET_PAYMENTS => tr('chat_asset_payments'),
        ORDER_PAYMENTS => tr('order_payments'),
        LIVE_VIDEO_PAYMENTS => tr('live_video_payments'),
        ALL_PAYMENTS => tr('all_payments')
    ];

    return isset($platform_list[$platform]) ? $platform_list[$platform] : tr('na');
}

function collection_post_user_needs_to_pay($collection_id, $user_id) {

    $is_user_needs_to_pay = NO;

    $collection = Collection::firstWhere(['id' => $collection_id, 'user_id' => $user_id]);

    $is_payment_done = CollectionPayment::where(['collection_id' => $collection_id, 'user_id' => $user_id, 'status' => PAID])->count();

    $is_user_needs_to_pay  = !$is_payment_done && !$collection ? YES : NO; 
